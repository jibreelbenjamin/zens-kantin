...
export const columns: ColumnDef<SpecialIncome>[] = [
  {
  ...
  { accessorKey: "nama", header: ({ column }) => <DataTableColumnHeader column={column} title="Pemasukan" /> },
  {
    accessorKey: "nominal",
    header: ({ column }) => <DataTableColumnHeader column={column} title="Nominal" />,
    cell: ({ row }) => <span className="tabular-figures font-medium text-success">+{formatRupiah(row.original.nominal)}</span>,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex justify-end gap-1">
        <IncomeFormDialog income={row.original} />
        <DeleteIncomeButton id={row.original.id} nama={row.original.nama} />
      </div>
    ),
  },
];
