...
/** Pemasukan Khusus (v9) — kebalikan Pengeluaran Khusus, menambah untung di laporan. */
export async function POST(request: Request) {
  try {
    ...
    if (!nama || !nama.trim() || Number(nominal) < 0) {
      return NextResponse.json({ error: "Data pemasukan khusus tidak valid." }, { status: 400 });
    }
    ...
    await logActivity({
      ...
      deskripsi: `Menambah pemasukan khusus ${nama}`, request,
    });
